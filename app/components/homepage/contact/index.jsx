...
function ContactSection() {
  const changeInfo = (info, type) => {
    ...
    if (type === "email") {
      secret = "inanda"
      partInfo = info.split("@")[0]
    }
    const lastInfo = partInfo.substring(partInfo.length - 6)
    const newLastInfo = lastInfo === "*****" ? secret : "*****"
    return info.replace(lastInfo, newLastInfo)
  }
          ...
          <div className="flex flex-col gap-5 lg:gap-9">
            <p className="text-sm md:text-xl flex items-center gap-3">
              <MdAlternateEmail
                ...
                size={36}
              />
              {/* <span onClick={toggleEmail}>{email}</span> */}
              <span data-tooltip-id='hidden-info-tooltip' data-tooltip-place='top' onMouseDown={() => setIsShowEmail(!isShowEmail)} onMouseUp={() => setIsShowEmail(!isShowEmail)}>
                {!isShowEmail === true ? changeInfo(personalData.email, "email") : personalData.email}
              </span>
            </p>
            <p className="text-sm md:text-xl flex items-center gap-3">
              <IoMdCall
